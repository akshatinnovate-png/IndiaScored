...
const ProfileForm: React.FC = () => {
  ...
  const [, setIsValidAadhaar] = useState<boolean | null>(null);
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        // Only the four fields the backend persists come back; the KYC
        // fields below stay local to this form until the Aadhaar step runs.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!user) throw new Error("You must be signed in to save a profile.");

      await api.profile.save(user.id, {
        name: profile.name,

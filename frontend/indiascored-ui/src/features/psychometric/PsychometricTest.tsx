...
interface SingleChoiceProps {
  ...
  options?: string[];
  checkedValue?: number;
  onChange: (qId: number, optionIndex: number) => void;
  disabled: boolean;
}
...
export default function BehavioralPsychometricTest() {
  ...
  useEffect(() => {
    ...
    const fetchStatus = async () => {
      try {
        const data = await api.psychometric.status(user.id);
        if (data.completed && data.taken_at) {
          const lastTest = new Date(`${data.taken_at}Z`);
          const now = new Date();
  ...
  async function saveScore(compositeScore: number) {
    if (!user) return;
    try {
      await api.psychometric.save(user.id, compositeScore);
      setSubmitted(true);
    } catch (err) {
      console.error("Error saving psychometric score:", err);
      alert(
        err instanceof Error
          ? `Could not save your assessment: ${err.message}`
  ...
  const LikertScale = ({ qId, checkedValue, onChange, disabled }: LikertScaleProps) => (

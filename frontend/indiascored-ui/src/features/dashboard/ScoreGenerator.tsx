...
interface IndiaScoreFormProps {
  onScoreGenerated?: (score: number, data: Record<string, unknown>) => void;
}

interface IndiaScoreFormData {
...
export function ScoreGenerator({ onScoreGenerated }: IndiaScoreFormProps) {
  const [formData, setFormData] = useState<IndiaScoreFormData>({
    user_type: "",
    region: "",
    sms_count: "",
    bill_on_time_ratio: "",
  ...
  const calculateIndiaScore = () => {
    setIsCalculating(true);
    
    // Simulate API call with IndiaScore algorithm
    setTimeout(() => {
      let baseScore = 300;
      const factors = {
        technology: 0,
  ...
  const handleInputChange = (field: keyof IndiaScoreFormData, value: string) => {
    ...
    <Card className="shadow-medium border-border/50">
      <CardHeader className="bg-gradient-boi text-primary-foreground">
        <div className="flex items-center gap-3">
          ...
          <div>
            <CardTitle className="text-xl font-bold">IndiaScore Generator</CardTitle>
            <p className="text-primary-foreground/80 text-sm">
              Generate comprehensive credit scores using IndiaScored's advanced algorithm
            </p>
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-6 pt-6">
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            ...
            <Button 
              onClick={calculateIndiaScore} 
              className="w-full bg-gradient-boi hover:opacity-90"
              disabled={isCalculating || !isFormValid()}
              size="lg"
            >
              {isCalculating ? "Generating IndiaScore..." : "Generate IndiaScore"}
            </Button>
          </div>
            ...
            {isCalculating && (
              <Card className="p-6 text-center border-border/50">
                <div className="space-y-4">
                  ...
                  <p className="text-muted-foreground">Processing with IndiaScore AI...</p>
                ...
                <Card className={`p-6 text-center border-l-4 ${scoreResult.riskCategory === 'Low' ? 'border-l-success bg-success/5' : scoreResult.riskCategory === 'Medium' ? 'border-l-warning bg-warning/5' : 'border-l-destructive bg-destructive/5'}`}>
                  <div className="space-y-3">
                    <div className="flex items-center justify-center gap-2">
                      <Badge className="bg-gradient-boi text-primary-foreground">IndiaScore</Badge>
                    </div>
                    <div className="text-4xl font-bold text-foreground">{scoreResult.indiaScore}</div>
                    <Badge 
            ...
            {!scoreResult && !isCalculating && (
              <Card className="p-6 text-center border-dashed border-border">
                ...
                <p className="text-muted-foreground">
                  Complete the form to generate your IndiaScore assessment
                </p>
              </Card>
            )}
          </div>
